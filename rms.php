...
function is_manager_user()
	...
	{
		$this->query = "
		SELECT MAX( CAST(`order_number` AS UNSIGNED) )AS order_number FROM order_table
		";
		...
		foreach($result as $row)
		{
			...
			else
			{
				...
				return sprintf('%04s', $row["order_number"] + 1);
	...
	public function Get_total_departments()
	{
		...
		$result = $this->get_result(); // Assuming `get_result()` handles the query execution and returns the result
	...
	public function Get_department_program_count()
	{
		...
		foreach ($result as $row)
		{
			// Check if programcount is null before adding to the result
			...
			{
				// If programcount is null, set it to '0'
				$departments[] = [
					'department' => $row['department'],
					'programcount' => '0'
				];
...
public function Get_department_list()
{
    ...
    foreach ($result as $row)
    {
        // Check if department and count are not null before adding to the result
        ...
        {
            // If department or count is null, you can handle it by assigning defaults
            $departments[] = [
                'department' => 'Unknown',
                'countt' => '0'
            ];
...
public function Get_chart7_department_list()
{
	...
	function Get_total_today_sales()
	{
		...
		WHERE order_date = '".$date."' and order_status=2
		";
		...
		foreach($result as $row)
		{
			...
			{
				return $this->cur . number_format($row["total"],2);
			}
			else
			{
				return '0';
...
//		echo $date;
	...
	{
		$date = date('d-m-Y');
$date=date('m-d-Y',(strtotime ( '-7 day' , strtotime ( $date) ) ));
		$this->query = "
		SELECT SUM(order_net_amount) AS total FROM order_table 
		WHERE order_date >= '".$date."' and order_status=2
		";
	...
	{
		$this->query = "
		SELECT SUM(order_net_amount) AS total FROM order_table where order_status=2
		";
...
//$conn = mysqli_connect("185.193.17.130", "chedro9mobt_trialuser98", "Tim_09102022", "chedro9mobt_rms");
