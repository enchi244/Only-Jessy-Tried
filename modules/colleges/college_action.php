...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'fetch')
	{
		...
		foreach($result as $row)
		{
			...
			{
				$status = '<button type="button" name="status_button" class="btn btn-success btn-sm status_button" data-id="'.$row["category_id"].'" data-status="'.$row["category_status"].'">Enable</button>';
			}
			else
			{
			...
			$sub_array[] = '
			<div align="center">
			<button type="button" name="edit_button" title="Update Category" style="margin-left: 5px; margin-bottom: 5px; margin-top:5px;" data-toggle="tooltip" class="btn btn-primary btn-sm edit_button" name="edit_button"  data-id="'.$row["category_id"].'"><i class="fas fa-pencil-alt"></i></button>
			
			<button type="button" name="delete_button" title="Delete Category" style="margin-left: 5px;" data-toggle="tooltip" class="btn btn-danger btn-sm delete_button" data-id="'.$row["category_id"].'" data-status="'.$row["category_status"].'"><i class="far fa-trash-alt"></i></button>
			</div>
			';
			$data[] = $sub_array;
		}
