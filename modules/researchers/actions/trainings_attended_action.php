        ...
        if(isset($files['name']) && is_array($files['name'])) {
            ...
            for($i = 0; $i < count($files['name']); $i++) {
                if($files['error'][$i] == 0) {
                    ...
                    $new_name = 'TRN_' . $safe_name . '_' . time() . '_' . rand(100, 999) . '.' . $ext;
...
if (isset($_POST["action_training"])) {
    ...
    if ($_POST["action_training"] == 'fetch_all') {
        $order_column = array('tbl_researchdata.familyName', 'tbl_trainingsattended.title', 'tbl_trainingsattended.venue', 'tbl_trainingsattended.date_train', 'tbl_trainingsattended.lvl');
        ...
        foreach ($result as $row) {
            ...
            $sub_array[] = $row["lvl"];
            $sub_array[] = '<div align="center"><button type="button" class="btn btn-danger btn-sm delete_master_training" data-id="'.$row["id"].'" title="Delete"><i class="far fa-trash-alt"></i></button><a href="view_researcher.php?id='.$row["author_db_id"].'&tab=tra" class="btn d-none"></a></div>';
            $data[] = $sub_array;
        }
    ...
    if ($_POST["action_training"] == 'fetch') {
        ...
        if (isset($_POST["search"]["value"])) {
            $search_value = $_POST["search"]["value"];
            $search_query .= "AND (title LIKE '%" . $search_value . "%' OR type LIKE '%" . $search_value . "%' OR venue LIKE '%" . $search_value . "%' OR sponsor_org LIKE '%" . $search_value . "%') ";
        }
        ...
        foreach ($result as $row) {
            ...
            $sub_array[] = '
            <div align="center">
                <button type="button" title="Edit Training" style="margin-left: 5px; margin-bottom: 5px; margin-top:5px;" class="btn btn-primary btn-sm edit_button_training" data-id="' . $row["id"] . '"><i class="fas fa-pencil-alt"></i></button>
                <button type="button" title="Delete Training" style="margin-left: 5px;" class="btn btn-danger btn-sm delete_button_training" data-id="' . $row["id"] . '"><i class="far fa-trash-alt"></i></button>
            </div>';
            $data[] = $sub_array;
        }
        echo json_encode(array("draw" => intval($_POST["draw"]), "recordsTotal" => $total_rows, "recordsFiltered" => $filtered_rows, "data" => $data));
    ...
    if ($_POST["action_training"] == 'Add') {
        ...
        $date_applied = date("m-d-Y", strtotime($_POST['date_training'])); // Legacy preservation
        ...
        (researcherID, title, type, venue, date_train, lvl, type_learning_dev, sponsor_org, totnh, has_files) 
        VALUES 
        (:researcherID, :title, :type, :venue, :date_train, :lvl, :type_learning_dev, :sponsor_org, :totnh, :has_files)
        ";
        $object->execute($data);
        $new_tid = $object->connect->lastInsertId();
        ...
        if($has_files == 'With' && isset($_FILES['training_files'])) {
            ...
            handle_training_files($object, $new_tid, $categories, $_FILES['training_files']);
        }
    ...
    if ($_POST["action_training"] == 'delete') {
        ...
        $object->query = "SELECT file_path FROM tbl_training_files WHERE training_id = '".$tid."'";
        $files_to_delete = $object->get_result();
        foreach($files_to_delete as $file) {
            $physical_path = '../../../' . $file['file_path'];
            if(file_exists($physical_path)) { unlink($physical_path); }
        }
        $object->query = "DELETE FROM tbl_training_files WHERE training_id = '".$tid."'";
        $object->execute();

        $object->query = "DELETE FROM tbl_trainingsattended WHERE id = '".$tid."'";
        $object->execute();
