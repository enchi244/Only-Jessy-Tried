...
if(isset($_POST["action"])) {
	...
	if($_POST["action"] == "fetch_item_details") {
        ...
        // 100% Bulletproof Absolute URL for the uploads folder
        $doc_path = $object->base_url . "uploads/documents/";

		// =========================================================================
		// RESEARCHER PORTFOLIO - Scanning across all modules perfectly!
		// =========================================================================
		if($type == 'researchers') {
			...
			foreach($result as $row) {
				...
				$object->query = "SELECT title, stat, started_date, moa_file FROM tbl_researchconducted WHERE researcherID = '$id'";
				...
				if(count($res1) > 0) {
					...
					foreach($res1 as $r1) { 
                        ...
                        $html .= "<li class='mb-2'><strong>" . ($r1['title'] ?: 'Untitled Project') . "</strong> <span class='badge badge-secondary ml-1'>{$r1['stat']}</span> {$moa_btn}<br><small class='text-muted'>Started: {$r1['started_date']}</small></li>"; 
                    }
					$html .= "</ul>";
				...
				$object->query = "SELECT title, type, date_granted, moa_file FROM tbl_itelectualprop WHERE researcherID = '$id'";
				...
				if(count($res3) > 0) {
					...
					foreach($res3 as $r3) { 
                        ...
                        $html .= "<li class='mb-2'><strong>" . ($r3['title'] ?: 'Untitled IP') . "</strong> <span class='badge badge-info ml-1'>{$r3['type']}</span> {$moa_btn}<br><small class='text-muted'>Granted: {$r3['date_granted']}</small></li>"; 
                    }
					$html .= "</ul>";
		...
		// =========================================================================
		// REGULAR MODULE DETAILS (Fixed absolute paths for all MOA links)
		// =========================================================================
		elseif($type == 'research_conducted') {
			$object->query = "SELECT r.*, d.firstName, d.familyName FROM tbl_researchconducted r JOIN tbl_researchdata d ON r.researcherID = d.id WHERE r.id = '$id'";
			$result = $object->get_result();
			foreach($result as $row) {
